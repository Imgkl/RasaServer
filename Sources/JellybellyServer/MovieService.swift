...
final class MovieService {
    ...
    func getMovies(limit: Int = 50, offset: Int = 0, includeTags: Bool = false) async throws -> MoviesListResponse {
    ...
    func reconfigureJellyfin(baseURL: String, apiKey: String, userId: String) {
        ...
        self.jellyfinService = JellyfinService(baseURL: baseURL, apiKey: apiKey, userId: userId, httpClient: client)
    ...
    private func attemptAutoLoginAndUpdate() async throws -> Bool {
        ...
        guard let url = try await store.get("jellyfin_url"),
              ...
              let encPwd = try await store.get("jellyfin_password_enc") else {
            return false
        }
        // Decrypt password
        ...
        do {
            let auth = try await JellyfinService.login(baseURL: url, username: username, password: password, httpClient: http)
            // Save new token
    ...
    func getMovies(withTag tagSlug: String, limit: Int = 50, offset: Int = 0) async throws -> MoviesListResponse {
    ...
    func updateMovieTags(movieId: String, tagSlugs: [String], replaceAll: Bool) async throws -> MovieResponse {
    ...
    func generateAutoTags(
        ...
        if chosen == nil || chosen == "auto" {
            ...
            switch chosen! {
            case "anthropic":
                guard let apiKey = config.anthropicApiKey, !apiKey.isEmpty else { throw MovieServiceError.missingApiKey("Anthropic") }
        ...
        let looksGeneric = Set(response.suggestions).contains("dialogue-driven") || Set(response.suggestions).contains("modern-masterpieces")
    ...
    private func postFilterSuggestions(_ suggestions: [String], summary: String?, movie: Movie) -> [String] {
        guard let text = summary?.lowercased() ?? movie.overview?.lowercased() else { return suggestions }
        var result: [String] = []
        let words = text
        ...
        for tag in suggestions {
            ...
            case "time-twists":
                ...
                let ok = [
                    "time travel","time-travel","time loop","timeloop","looping time","resetting day","timeline",
            ...
            case "psychological-pressure-cooker":
                ...
                let hasOneRoom = ["single room","one room","jury room","confined space","bottle episode","bottle film"].contains(where: { words.contains($0) })
                let hasPsych = ["paranoia","psychological","mental breakdown","gaslight","psychosis","claustrophobic"].contains(where: { words.contains($0) })
    ...
    func searchMovies(query: String, limit: Int = 20, includeTags: Bool = false) async throws -> MoviesListResponse {
        ...
        let movies = try await searchQuery
            .sort(\.$title)
            .limit(limit)
            .all()
    ...
    func searchTags(query: String) async throws -> TagsListResponse {
        ...
        let matchingBuckets = config.moodBuckets.filter { slug, bucket in
            slug.contains(query.lowercased()) ||
            bucket.title.lowercased().contains(query.lowercased()) ||
    ...
    func getPlaybackUrls(movieId: String, format: String) async throws -> PlaybackUrlsResponse {
        ...
        let subtitleTracks = movie.jellyfinMetadata?.mediaStreams
        ...
        let audioTracks = movie.jellyfinMetadata?.mediaStreams
    ...
    func importTagsMap(_ map: [String: ExportMovieTags], replaceAll: Bool = true) async throws {
        for (movieKey, payload) in map {
            do {
                ...
                do {
                    _ = try await updateMovieTags(movieId: movieKey, tagSlugs: payload.tags, replaceAll: replaceAll)
                    continue
                } catch {
                ...
                if let byTitle = try await titleQuery.first() {
                    ...
                    _ = try await updateMovieTags(movieId: idString, tagSlugs: payload.tags, replaceAll: replaceAll)
                } else {
                    logger.error("Import: No movie found for key=\(movieKey) or title=\(title)")
                }
            } catch {
                logger.error("Failed to import tags for movie key=\(movieKey) title=\(payload.title): \(error)")
            }
        }
    ...
    func syncWithJellyfin(fullSync: Bool = false) async throws -> SyncStatusResponse {
        ...
        do {
            ...
            logger.info("Jellyfin sync completed: \(stats.moviesFound) found, \(stats.moviesUpdated) updated, \(stats.moviesDeleted) deleted")
    ...
    func testJellyfinConnection() async throws -> ConnectionTestResponse {
        do {
            ...
            if isConnected {
                ...
                return ConnectionTestResponse(success: false, serverInfo: nil, error: "Authentication failed")
            }
        } catch {
            return ConnectionTestResponse(success: false, serverInfo: nil, error: error.localizedDescription)
        }
    }
